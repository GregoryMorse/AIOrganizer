...
runtime first and the official SDK's pinned `codex_cli_bin` app-server runtime as
the packaged fallback. Codex runs in an empty working directory, read-only sandbox,
deny-all approval mode, disabled web search and shell network, with only
AIOrganizer's proposal MCP configured.
